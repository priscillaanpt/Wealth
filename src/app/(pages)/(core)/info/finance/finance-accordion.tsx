...
import AccordionInput from "~/components/info/finance/accordion-input";
import {
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
  ...
  return (
    <AccordionItem {...accProps} className="rounded-xl bg-white">
      <AccordionTrigger className="grid grid-cols-[16px_300px_1fr_1fr] gap-0 p-6">
        ...
        <p className="text-b1 text-blacknavy px-2 text-right text-sm">Rp {formatToRupiah(totalValuePerMonth.toFixed(2).toString())} / Bulan</p>
        <p className="text-b1 text-blacknavy px-2 text-right text-sm">Rp {formatToRupiah((totalValuePerMonth * 12).toFixed(2).toString())} / Tahun</p>
      </AccordionTrigger>
          ...
          <TableBody>
            {Object.keys(fieldData).map((keyVal, idx) => { 
              ...
              <AccordionInput
                ...
                onDataChange={(val) => setIncomeProp(keyVal, val as TFieldData[string])}
                onMonthChange={(val) => setMonthProp(keyVal+"Months", val as TMonthData[string])}
              />
            )})}
          </TableBody>
        </Table>
